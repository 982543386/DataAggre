...
function deleteTemplateItem(templateID){
	var confirm = confirmAct();
	if(confirm){
	var params = {};
	params["templateID"] = templateID;
	$.ajax({
		url : "/iiot/project/application/delete",
		type : "POST",
		data :params,
		...
		success : function(response) {
			if (response.status > -1) {
				getTemplateTable();
				gobackMsg(response.message, "Template <strong>" + name
						+ "</strong> deleted successfully.",
						'iiot/app/edge_app_templates.html');
...
function initAppDeploy(){
	$.ajax({
		url : "/iiot/project/names",
		type : "GET",
		...
		success : function(data) {
			...
			for (i = 0; i < data.data.length; i++) {
				$("#templates").append(
						"<option value='"+data.data[i].projectId+"'>"
...
function getTemplateTable(){
	...
	templateTable =*/ 
	 templateTable = $('#templateTable').DataTable({
        "ajax":"/iiot/project/application/list",
        "columns": [
			...
			},
			{
				targets : 4,
				title : "操作",
				"defaultContent" : "<button class='btn btn-primary btn-sm view'>查看</button>" +
						"&nbsp;<button class='btn btn-danger btn-sm delete'>删除</button>" +
						"&nbsp;<button class='btn btn-info btn-sm deployAppToNodes' data-toggle='modal' data-target='#modalDeploy'>部署</button>"
			},
			{
				targets : 5,
				title : "序号",
...
function getEdgeAppList(){
	 table = $('#appTable').DataTable({
	autoWidth: false,
	responsive: true,
    ajax:{
		url:"/iiot/project/app/list",
    ...
    "columns": [
								...
								},
								{
									...
									render: function (status) {
										if(status==0){
											return "<span class='btn-block bg-red btn-sm label-status'>未部署</span>";
										}else if(status==1){
											return "<span class='btn-block bg-green btn-sm label-status'>部署中</span>";
										}else if(status==2){
											return "<span class='btn-block bg-green btn-sm label-status'>已部署</span>";
										}else{
											return "<span class='btn-block bg-yellow btn-sm label-status'>未知</span>";
										}
								...
								},
								{
									targets : 7,
									title : "监控",
									"defaultContent" : /*"<button class='btn btn-primary btn-sm deploy'>部署</button>&nbsp;" +*/
											"<button class='btn btn-info btn-sm status'>监控</button>"
								}
								/*,
								 { "data": "template"	}, 
...
function checkTemplateInput(form) {
	var scroll = true;
    var ret = true;
...
function addIiotProject() {
	debugger
	alert('addIIOT');
	if (checkTemplateInput('#ku8form')) {
		var name = $('#edge_name').val();
		var version = $('#edge_version').val();
		var note = $('#edge_note').val();
		var json = {};
		json.projectName = name;
		json.version = version;
		json.notes = note;
		json.services = createServiceJson();
		// alert(json);
		...
		/*
		 * alert(JSON.stringify(json)); var json1 = {}; json1["name"] = name;
		 * alert(JSON.stringify(json1));
		 */

		$.ajax({
			url : "/iiot/project/application/add",
			type : "POST",
			data : params,
            ...
            $.each(img.data, function(index_i, item) {
                ...
                if (_imgId === undefined || filter !== '' || _imgId === -1) {
                    if (index_i === 0) {
                        ...
                        $(caller).html(item.imageName).data('imgId', item.id);
                    }
                } else {
                    if (item.id === _imgId) {
                        dockerDiv.addClass('selected');
                        imgSuccess.addClass('selected');
                        $(caller).html(item.imageName);
                    }
                }
...
function fun1(){
    var caller = $("#service_panel_0").find('#conImage');
    getDockerImage("",caller);
}

function gobackMsg(title, message, url) {
...
function addEdgeService(obj) {
    var uid = new Date().getUTCMilliseconds();
    var service = $(obj).closest(".form-group[name='edge_service']");
