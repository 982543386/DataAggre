 ...
 */
@Service
public class ProjectToK8Service {
	...
	public io.fabric8.kubernetes.api.model.Service buildService(int clusterId, String namespace, boolean isPublic, String appName, org.ku8eye.bean.project.Service s, User user) throws KubernetesClientException
	{
			...
			//Setup for Service Label, add APPTYPE and APPNAME  20190808 删除label
			...
			try
			{
				...
				io.fabric8.kubernetes.api.model.Service response_f8Service = k8APIService.createService(clusterId, namespace, f8Service);
	...
	public ReplicationController buildReplicationController(int clusterId, String namespace, boolean isPublic, String appName, org.ku8eye.bean.project.Service s, User user) throws KubernetesClientException
	{
		...
		try
		{
			...
			ReplicationController response_f8ReplicationController = k8APIService.createReplicationController(clusterId, namespace, replicationController);
	...
	public Deployment buildDeployment(String nodeName, String namespace, boolean isPublic, String appName, org.ku8eye.bean.project.Service s, User user) throws KubernetesClientException
	{
		if(log.isInfoEnabled())
			log.info("[USER: " + user.getUserId() + "] Creating RC:" + s.getName());
		...
		labelSelector.setMatchLabels(sp_map);
		try
		{
			Deployment deployment = new DeploymentBuilder().withKind("Deployment")
					.withNewMetadata()
					.withName(s.getName())
					.withNamespace(namespace)
					.withLabels(l_map)
			...
			Deployment response_f8Deployment = k8APIService.createDeployment(clusterId, namespace, deployment);
			return response_f8Deployment;
		}
		catch(IllegalStateException ise)
		{
			String msg = "RC \"" + s.getName() + "\" failed to build, " + ise.getMessage();
		...
		for (org.ku8eye.bean.project.Volume ku8Volume : ku8VolumeArray)
		{
			...
			try
			{
				Volume f8Volume = new VolumeBuilder()
					...
					//TODO
					.withHostPath(new HostPathVolumeSource(ku8Volume.getPath(),"Directory"))
					.build();
	...
	private Container[] createContainers(String serviceName, org.ku8eye.bean.project.Container containerArray[], User user) throws KubernetesClientException
	{
		...
		for (int i = 0; i < containerArray.length; i++)
		{
			...
			//TODO
			//DockerImage dImg = imageService.getImagesId(7135);
			if(dImg == null)
			{
	...
	private void createConfigMaps(int clusterId, String namespace, String serviceName, org.ku8eye.bean.project.Volume ku8ConfMapArray[], Container f8Containers[], List<Volume> f8Volumes, User user) throws KubernetesClientException
	{
		...
		for (org.ku8eye.bean.project.Volume confMap : ku8ConfMapArray)
		{
			...
			ConfigMap f8ConfigMap = k8APIService.getConfigMapByName(clusterId, namespace, confMap.getName());
