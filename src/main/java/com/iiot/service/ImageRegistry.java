...
import com.iiot.domain.dockerImage.DockerImageImportBeanArray;
import com.iiot.utils.DBOperator;
import com.iiot.utils.SystemUtil;
import com.iiot.utils.dockerImage.CommonUtil;
import com.iiot.utils.dockerImage.FileUtil;
...
public class ImageRegistry {
	...
	public int executeInsertImage(DockerImageImportBean theImage)
			throws Exception {
		return DBOperator.executeInsertImage(theImage);
	}
	 ...
	 */
	@Transactional(propagation = Propagation.REQUIRED)
	public int executeUpdateImage(DockerImageImportBean theImage)
			throws Exception {
		return DBOperator.executeUpdateImage(theImage);
	}
	 ...
	 * 里的所有DockerImage信息， 图片路径存放在ku8_ext_filesPath下的logo_pic
	 * 中（DockerImage里存放logo_pic/xxx.png这样的相对路径），image文件cp到
